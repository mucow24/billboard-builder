...
export const zigzagsGeneratorSpec: GeneratorSpec<ZigzagsGeneratorParams> = {
  ...
  draw(ctx, w, h, params, seed) {
    if (params.zigzagCount <= 0 || params.zigzagOpacity <= 0 || params.zigzagThickness <= 0) return;
    const rng = mulberry32(params.seedOverride ?? seed);
    ctx.save();
    ctx.lineCap = 'butt';
    ctx.lineJoin = 'miter';
    ctx.globalAlpha = params.zigzagOpacity;
    for (let i = 0; i < params.zigzagCount; i++) {
      const y = ((i + 0.5) / params.zigzagCount) * h;
      ...
      ctx.lineWidth = params.zigzagThickness;
      ctx.stroke();
    }
    ctx.restore();
  },
