...
| GD-14 | Canvas | Grouped child is fully off-canvas but visible in the unclipped workspace | Click grouped content once, then click the same child again | Group affordance yields to child affordance | Off-canvas child drill-in follows the direct item-hit path | user-flow | Chromium | Covered in [e2e/editor.groups.spec.ts](/home/mikek/src/billboard-builder/e2e/editor.groups.spec.ts) |
