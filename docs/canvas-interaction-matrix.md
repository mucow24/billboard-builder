...
| CS-15 | Canvas | Select tool, overflow preview visible fully outside the canvas | Drag marquee entirely outside the canvas | No selection affordance appears | Overflow preview content is not selected | user-flow | Chromium | Covered in [e2e/editor.entrypoints.spec.ts](/home/mikek/src/billboard-builder/e2e/editor.entrypoints.spec.ts) |
| CS-16 | Canvas | Select tool, one item partially overlaps canvas and another is fully outside | Drag marquee across the canvas edge | Only the canvas-overlapping item shows selection affordance | Marquee selection is clipped to canvas bounds before hit testing | user-flow | Chromium | Covered in [e2e/editor.entrypoints.spec.ts](/home/mikek/src/billboard-builder/e2e/editor.entrypoints.spec.ts) |
